...
function cell(t, shd) { return { runs: t ? [{ text: t }] : [], kind: 'cell', shd: shd }; }
function rowEnd(t, shd) { return { runs: t ? [{ text: t }] : [], kind: 'rowEnd', shd: shd, tblw: FULL }; }
...
function model(bookmarks) {
  return {
    ...
    body: [].concat(
      ...
      // A status table: merged header, shaded State cells, an empty Owner cell, a part-width merge.
      [{ runs: [{ text: 'Project status — Q2', b: true }], kind: 'rowEnd', shd: GREY, tblw: [0, 9000] }],
      [cell('Phase'), cell('State'), rowEnd('Owner')],
      [cell('Design'), cell('Done', GREEN), rowEnd('Alpaq92')],
      [cell('Build'), cell('Blocked', RED), rowEnd('Alpaq92')],
